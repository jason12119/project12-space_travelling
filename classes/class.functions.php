...
class functions
{
    ...
    public function parseCenik($cenik)
    {
        if ($cenik == NULL) {
            exit();
        }
        for ($i = 0; isset($cenik[$i]); $i++) {
            echo '<tr>';
            echo "<td>" . $cenik[$i]->typ . "</td>";

            if ($cenik[$i]->trida == NULL) {
                $cenik[$i]->trida = "";
            } else {
                $cenik[$i]->trida = "tř. " . $cenik[$i]->trida;
            }
            echo "<td>" . $cenik[$i]->trida . "</td>";

            $cenik[$i]->cena = $this->replaceDot($cenik[$i]->cena);

            echo "<td>" . $cenik[$i]->cena . " Kč" . "</td>";
            echo '</tr>';

        }
    }

    public function replaceDot($float)
    {
