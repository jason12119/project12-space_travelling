...
class data extends db_connect
{
    ...
    public function getMetaTags()
    {
        ...
        if ($_GET['page']) {
            ...
            $sql =
                'SELECT title, description, keywords FROM ' .
                $this->prefix .
                "menu WHERE url = '$url'";
    ...
    public function getMenuItems($header = null, $footer = null, $pod = null)
    {
        ...
        $sql =
            'SELECT id, nazev, nazev_en, nazev_de, nazev_sk, url, ikona, foto, obsah, id_fotogalerie FROM ' .
            $this->prefix .
            "menu WHERE aktivni='1' $head $foot AND !pod ORDER BY poradi ASC";

        if ($pod) {
            $sql =
                'SELECT id, nazev, nazev_en, nazev_de, nazev_sk, url, ikona, foto, obsah, id_fotogalerie FROM ' .
                $this->prefix .
                "menu WHERE aktivni='1' AND pod='$pod' AND !vnoreno ORDER BY poradi ASC";
        }
    ...
    public function showMenuItems()
    {
        ...
        foreach ($this->menuItems as $item) {
            $menuItem .=
                '<li><a class="scrollTo" data-target="#' .
                $item->url .
                '" href="' .
                $item->url .
                '/">' .
    ...
    public function getContent($url)
    {
        ...
        $sql =
            'SELECT id, nazev, perex, nazev_en, nazev_de, nazev_sk, obsah, obsah_en, obsah_de, obsah_sk, url, pod, foto, ikona, h1, id_fotogalerie FROM ' .
            $this->prefix .
            "menu WHERE url = '$url' AND aktivni='1'"; //  AND !vnoreno
        $result = $this->db()->query($sql);
        $row = mysqli_fetch_object($result);
        ...
        $sql1 =
            'SELECT id, nazev, perex, nazev_en, nazev_de, nazev_sk, obsah, obsah_en, obsah_de, obsah_sk, url, pod, foto, ikona, h1, id_fotogalerie FROM ' .
            $this->prefix .
            "menu WHERE pod = '$row->id' AND aktivni='1' AND vnoreno";
        ...
        $sql1 =
            'SELECT file, nazev FROM ' .
            $this->prefix .
            "menu_dokumenty WHERE id_menu = '$row->id' ORDER BY poradi ASC";
        $result1 = $this->db()->query($sql1);
        while ($row1 = mysqli_fetch_object($result1)) {
            $files[] = $row1;
        }
    ...
    public function getContentById($id)
    {
        ...
        $sql =
            'SELECT id, nazev, nazev_en, nazev_de, nazev_sk, obsah, obsah_en, obsah_de, obsah_sk, url, foto, ikona, h1,id_fotogalerie FROM ' .
            $this->prefix .
            "menu WHERE id = '$id' AND aktivni='1'";
        $result = $this->db()->query($sql);
        $row = mysqli_fetch_object($result);

        if ($row->id_fotogalerie) {
            $sql1 =
                'SELECT url, nazev FROM ' .
                $this->prefix .
                "fotogalerie WHERE id='$row->id_fotogalerie' AND aktivni";
            $result1 = $this->db()->query($sql1);
    ...
    public function getCategoryData($url)
    {
        ...
        $sql =
            'SELECT id, nazev, nazev_en, nazev_de, nazev_sk, obsah, obsah_en, obsah_de, obsah_sk, url, foto, ikona, h1, pod FROM ' .
            $this->prefix .
            "kategorie WHERE url = '$url'";
        $result = $this->db()->query($sql);
        $row = mysqli_fetch_object($result);
    ...
    public function saveEmail($email, $uid)
    {
        ...
        if (!$result->num_rows) {
            $sql =
                'INSERT INTO ' .
                $this->prefix .
                "newsletter SET email='$email', uid='$uid'";
            $this->db()->query($sql);
            return true;
        } else {
            return false;
    ...
    public function getNews($limit = null)
    {
        ...
        $sql =
            'SELECT id, nazev, nazev_en, nazev_de, nazev_sk, perex, perex_en, perex_de, perex_sk, obsah, obsah_de, obsah_en, obsah_sk, foto, foto_en, foto_de, foto_sk, url, datum FROM ' .
            $this->prefix .
            "novinky WHERE aktivni = '1' ORDER BY poradi ASC $limit_cond";
        $result = $this->db()->query($sql);
        while ($row = mysqli_fetch_object($result)) {
            $data[] = [
                'id' => $row->id,
    ...
    public function downloadCNB()
    {
        ...
        $feed = file_get_contents(
            "http://www.cnb.cz/cs/financni_trhy/devizovy_trh/kurzy_devizoveho_trhu/denni_kurz.txt?date=$date"
        );
    ...
    public function getGalleries()
    {
		...
		" .
            ...
            'fotogalerie.id = ' .
            $this->prefix .
            "fotogalerie_media.id_gallery 
		WHERE 
		" .
            $this->prefix .
            'fotogalerie.aktivni AND (' .
            $this->prefix .
            "fotogalerie_media.poradi='1' OR " .
            $this->prefix .
            "fotogalerie_media.poradi='0')
		GROUP BY 
		" .
            $this->prefix .
            "fotogalerie.id 
    ...
    public function getReferences($id = null, $limit = null)
    {
		...
		" .
            ...
            'reference.id = ' .
            $this->prefix .
            "reference_media.id_reference 
		WHERE 
		$cond2 " .
            $this->prefix .
            'reference.aktivni AND (' .
            $this->prefix .
            "reference_media.poradi = '1' OR " .
            $this->prefix .
            "reference_media.poradi = '0')
		GROUP BY 
		" .
            $this->prefix .
            "reference.id 
    ...
    public function getGalleryPhotos($id, $limit = null)
    {
        ...
        $id = $this->sqlInjection($id);
        $sql =
            'SELECT foto, nazev FROM ' .
            $this->prefix .
            "fotogalerie_media WHERE id_gallery = '$id' ORDER BY poradi ASC $limit_cond";
        $result = $this->db()->query($sql);
        return $result;
    }
    ...
    public function getReferencesPhotos($id)
    {
        ...
        $sql =
            'SELECT foto, nazev FROM ' .
            $this->prefix .
            "reference_media WHERE id_reference = '$id' ORDER BY poradi ASC";
        $result = $this->db()->query($sql);
        return $result;
    }
    ...
    public function getNextPrevReference()
    {
        $sql =
            'SELECT id,url FROM ' .
            $this->prefix .
            'reference WHERE aktivni ORDER BY poradi ASC';
        $result = $this->db()->query($sql);
        while ($row = mysqli_fetch_object($result)) {
            $data[] = $row->url;
        }
    ...
    public function getFolders()
    {
        $data = [];
        $sql =
            'SELECT nazev, url, id FROM ' .
            $this->prefix .
            'dokumenty WHERE aktivni ORDER BY poradi ASC';
        $result = $this->db()->query($sql);
        while ($row = mysqli_fetch_object($result)) {
            $data[] = $row;
        }
    ...
    public function getFiles($folderId)
    {
        ...
        $sql =
            "SELECT
				" .
            $this->prefix .
            "dokumenty_media.nazev,
				" .
            $this->prefix .
            "dokumenty_media.file,
				" .
            $this->prefix .
            "dokumenty_media.id,
				" .
            $this->prefix .
            "dokumenty.nazev as folderName
				FROM " .
            $this->prefix .
            "dokumenty_media
            ...
            'dokumenty.id = ' .
            $this->prefix .
            "dokumenty_media.id_folder
				WHERE id_folder='" .
            $folderId .
            "'
				ORDER BY " .
            $this->prefix .
            'dokumenty_media.poradi ASC';
        $result = $this->db()->query($sql);
        $i = 0;
        while ($row = mysqli_fetch_object($result)) {
            $i++;
    ...
    public function getCalendar($limit = null)
    {
        ...
        $sql =
            'SELECT nazev, datum, popis FROM ' .
            $this->prefix .
            "kalendar WHERE aktivni ORDER BY datum DESC $cond";
        $result = $this->db()->query($sql);
        while ($row = mysqli_fetch_object($result)) {
            $data[] = $row;
        }
    ...
    public function search($value)
    {
        ...
        while ($row = mysqli_fetch_object($result)) {
            if ($row->pod && !$row->vnoreno) {
                $sql1 =
                    'SELECT id, url, vnoreno, pod FROM ' .
                    $this->prefix .
                    "menu WHERE id='" .
                    $row->pod .
                    "'";
        ...
        $sql =
            'SELECT * FROM ' .
            $this->prefix .
            "dokumenty_media WHERE $sql_condition_files";
        $result = $this->db()->query($sql);
        while ($row = mysqli_fetch_object($result)) {
            $data['dokumenty'][] = $row;
        }
